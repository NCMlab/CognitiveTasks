...
expInfo = {u'session': u'001', u'participant': u'', u'Font Size': u'unused'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/jasonsteffener/Dropbox/NeuralCognitiveMapping/DMSPsychopyFiles/DMS.psyexp',
...
trials1 = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('DMSFile.xlsx', selection=[0,1,2,3,4,5]),
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    frameRemains = 0.0 + 21.0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if textInterblock.status == STARTED and t >= frameRemains:
        textInterblock.setAutoDraw(False)
    
    # *text3A* updates
    if t >= 21.0 and text3A.status == NOT_STARTED:
        # keep track of start time/frame for later
        text3A.tStart = t
        text3A.frameNStart = frameN  # exact frame index
        text3A.setAutoDraw(True)
    frameRemains = 21.0 + 1.0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if text3A.status == STARTED and t >= frameRemains:
        text3A.setAutoDraw(False)
    
    # *text2A* updates
    if t >= 22.0 and text2A.status == NOT_STARTED:
        # keep track of start time/frame for later
        text2A.tStart = t
        text2A.frameNStart = frameN  # exact frame index
        text2A.setAutoDraw(True)
    frameRemains = 22.0 + 1.0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if text2A.status == STARTED and t >= frameRemains:
        text2A.setAutoDraw(False)
    
    # *text1A* updates
    if t >= 23.0 and text1A.status == NOT_STARTED:
        # keep track of start time/frame for later
        text1A.tStart = t
        text1A.frameNStart = frameN  # exact frame index
        text1A.setAutoDraw(True)
    frameRemains = 23.0 + 1.0- win.monitorFramePeriod * 0.75  # most of one frame period left
...
trials2 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[6,7,8,9,10,11]),
...
trials3 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[12,13,14,15,16,17]),
...
trials4 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[18,19,20,21,22,23]),
...
trials5 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[24,25,26,27,28,29]),
...
trials6 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[30,31,32,33,34,35]),
...
# set up handler to look after randomisation of conditions etc
trials7 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[36,37,38,39,40,41]),
...
for thisTrials7 in trials7:
    ...
    trials7.addData('resp.keys',resp.keys)
    trials7.addData('resp.corr', resp.corr)
    if resp.keys != None:  # we had a response
        trials7.addData('resp.rt', resp.rt)
    thisExp.nextEntry()
...
# set up handler to look after randomisation of conditions etc
trials8 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('DMSFile.xlsx', selection=[42,43,44,45,46,47]),
...
for thisTrials8 in trials8:
    ...
    trials8.addData('resp.keys',resp.keys)
    trials8.addData('resp.corr', resp.corr)
    if resp.keys != None:  # we had a response
        trials8.addData('resp.rt', resp.rt)
    thisExp.nextEntry()
